...
vz_new = vz.reshape(nsnaps,nx//8,8,nz//2,2).mean(axis=(2,4))
# x 768
for t in tqdm(range(nsnaps)):
    plt.clf()
    plt.figure(figsize=(20,12))
    plt.subplot(211)
    plt.imshow(bz_new[t,:,700//2:900//2].T*np.sqrt(4*np.pi),origin='lower',cmap='PuOr_r',norm=TwoSlopeNorm(vmin=-500,vcenter=0,vmax=500),extent=[0,1536*6.5,0-(110*6.5),200*6.5-(110*6.5)])
    ...
    plt.contour(bz_new[t,:,700//2:900//2].T*np.sqrt(4*np.pi),levels=[-50],colors=['brown'],extent=[0,1536*6.5,0-(110*6.5),200*6.5-(110*6.5)])
    plt.contour(tau[t,:,700:900].T,levels=[0.01,0.1,1],colors=['blue','green','red'],alpha=0.6,extent=[0,1536*6.5,0-(110*6.5),200*6.5-(110*6.5)])
    plt.xlim([800*6.5,1100*6.5])
    plt.ylim([50*6.5-110*6.5,170*6.5-110*6.5])
    plt.xlabel('x [km]')
    plt.ylabel('z [km]')
    plt.title('Bz [G]')
    plt.subplot(212)
    plt.imshow(-vz_new[t,:,700//2:900//2].T/1e5,origin='lower',cmap='bwr',norm=TwoSlopeNorm(vmin=-10,vcenter=0,vmax=10),extent=[0,1536*6.5,0-(110*6.5),200*6.5-(110*6.5)])
    plt.colorbar(aspect=40)
    ...
    plt.tight_layout()
    plt.savefig('/dat/xenosh/muram_plage_pore/vertical/b'+str(t)+'.png',bbox_inches='tight')
